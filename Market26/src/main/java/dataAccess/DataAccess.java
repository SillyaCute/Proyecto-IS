...
public class DataAccess  {
	...
	public void initializeDB(){
		...
		try { 
		    ...
		    //Create sellers 
			Seller seller1=new Seller("Aitor Fernandez", "seller1@gmail.com","123");
			Seller seller2=new Seller("Ane Gaztañaga", "seller2@gmail.com","321");
			Seller seller3=new Seller("Test Seller", "seller3@gmail.com", "000");
			ofertasBorradas=new Seller("Basura", "basuraBasurez@gmail.com", "basura");
	...
	public void crearUsuario(String nombre, String correo, String contraseña, String tipo) {
		...
		db.getTransaction().begin();
			...
			if(tipo.equals("Vendedor")) {
				Seller nuevoVendedor = new Seller(nombre, correo, contraseña);
				db.persist(nuevoVendedor);
				System.out.println("Vendedor guardado "+nuevoVendedor + nuevoVendedor.getClass());
			}else if(tipo.equals("Comprador")) {
				Comprador nuevoComprador = new Comprador(nombre, correo, contraseña);
				db.persist(nuevoComprador);
				System.out.println("Comprador guardado "+nuevoComprador + nuevoComprador.getClass());
			}
	...
	public void crearContraoferta(Contraoferta contra) {
		Sale compraBorrar = contra.getCompra();
		
		
		db.getTransaction().begin();
		compraBorrar = db.find(Sale.class, compraBorrar.getSaleNumber());
		Seller ofertasBorradasDB = db.find(Seller.class, "basuraBasurez@gmail.com");
		Seller Enrique = db.find(Seller.class, compraBorrar.getSeller().getEmail());

		Enrique.getSales().remove(compraBorrar);		
		compraBorrar.setSeller(ofertasBorradasDB);
		ofertasBorradasDB.getSales().add(compraBorrar);
		db.persist(contra);
	...
	public List<Contraoferta> getContraofertas(Seller mauricio){
		Seller malcom = db.find(Seller.class, mauricio.getEmail());
		System.out.println(malcom);
		TypedQuery<Contraoferta> query = db.createQuery("SELECT c FROM Contraoferta c",Contraoferta.class);
		List<Contraoferta> contras = query.getResultList();
		System.out.println("Contenido de la base de datos:");
		for (Contraoferta c:contras) System.out.println(c.toString());
		TypedQuery<Contraoferta> query1 = db.createQuery("SELECT c FROM Contraoferta c WHERE c.vendedor.email = :email", Contraoferta.class);
		query1.setParameter("email", malcom.getEmail());
